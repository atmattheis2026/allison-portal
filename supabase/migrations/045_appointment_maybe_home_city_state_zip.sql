-- Same city/state/zip split that Homes shown just got, added earlier in the
-- pipeline too — Appointments and Homes you may like — so it's captured
-- once wherever a home first shows up and carries forward automatically
-- instead of needing to be re-typed (or prompted for) at convert time.
alter table lead_appointments add column if not exists city_state_zip text;
alter table lead_maybe_homes add column if not exists city_state_zip text;
