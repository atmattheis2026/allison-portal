-- Heather's client page (/l/<token>) kept showing "Something went wrong"
-- after 071/072. Those versions (like 059 before them) named every column
-- directly: v_lead.loan_type_other, h.offer_requested, and so on. plpgsql
-- only checks those names when the function runs, so if the live database
-- is missing even one column from an older migration, every call fails and
-- the client page can't open.
--
-- This version can't fail that way:
--   * every row is read through to_jsonb(), so a missing column comes back
--     null instead of raising an error;
--   * each section (agent, lender, brand, appointments, homes, ...) is built
--     in its own guarded block, so one broken section (even a missing
--     table) becomes empty instead of taking the whole page down.
-- It returns exactly the same fields as 059 + the lender object from 071.
-- It still selects them key by key, so nothing new is exposed to anon.
