-- Company name and business NMLS for loan officers/mortgage brokers on the
-- roster — shown wherever that person is cast as the lender, same as the
-- lender websites already are.
alter table team_members add column if not exists company_name text;
alter table team_members add column if not exists nmls_number text;
