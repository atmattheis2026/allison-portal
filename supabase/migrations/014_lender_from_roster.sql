-- The loan officer card worked differently from the realtor card — typed in
-- fresh each time, with a "quick fill" shortcut bolted on top. Since her loan
-- people are always her own team (not outside lenders), this makes it work
-- exactly like Realtor: picked from the roster, so their name/NMLS/headshot
-- live on their team account and never get retyped or hunted for again.
--
-- Existing typed-in lenders are left alone (lender_member_id is null for
-- them) — get_shared_transaction() falls back to the old flat columns so
-- nothing already filled in disappears.

alter table transactions
  add column if not exists lender_member_id uuid references team_members(id) on delete set null;
