-- "Let's make an offer!" — a client-facing button on a completed appointment
-- that flags it and notifies both the assigned agent and (new) assigned
-- lender, mirroring the request-showing pattern on lead_maybe_homes.
alter table lead_appointments add column if not exists offer_requested boolean not null default false;
alter table lead_appointments add column if not exists offer_requested_at timestamptz;

-- Leads didn't have a lender concept before — everything pre-contract ran
-- through the agent only. Needed now so an offer request can notify the
-- lender too, same as transactions already do with lender_member_id.
alter table leads add column if not exists lender_member_id uuid references team_members(id) on delete set null;
  ...
  select jsonb_build_object(
    ...
    'appointments', coalesce((
      select jsonb_agg(jsonb_build_object(
               ...
               'completed', a.completed, 'offer_requested', a.offer_requested)
