-- Lets a client request a showing on a "Homes you may like" candidate
-- straight from their own page. Flag + timestamp only — the request-showing
-- edge function (anon-callable, no session, same trust model as
-- add_lead_referral) sets these and emails the assigned agent; the agent
-- sees a badge on the home and can schedule a real appointment from there.
alter table lead_maybe_homes add column if not exists showing_requested boolean not null default false;
alter table lead_maybe_homes add column if not exists showing_requested_at timestamptz;

-- Exposes showing_requested to the client too, so their own page correctly
-- shows "Showing requested" instead of the button again after a reload.
