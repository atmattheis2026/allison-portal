-- Original offer price + contingencies/addenda (deal terms, entered by her —
-- not MLS-sourced, so no "needs verification" disclaimer like Home Info) and
-- final purchase price, which replaces the closing countdown once a deal is
-- marked Closed & Funded.
alter table transactions add column if not exists offer_price numeric(12,2);
alter table transactions add column if not exists contingencies_addendums text;
alter table transactions add column if not exists final_purchase_price numeric(12,2);
...
begin
  ...
  select jsonb_build_object(
    'transaction', jsonb_build_object(
      ...
      'offer_price', v_tx.offer_price,
      'contingencies_addendums', v_tx.contingencies_addendums,
      'final_purchase_price', v_tx.final_purchase_price,
