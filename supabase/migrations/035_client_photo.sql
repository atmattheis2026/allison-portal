-- A second buyer (couples are the common case) gets their own name and
-- photo slot, both optional. Photos are uploaded by the client themselves,
-- from their own page — client-visible obviously (it's theirs), and useful
-- for the agent to see at a glance who they're working with too.
alter table leads add column if not exists full_name_2 text;
alter table leads add column if not exists client_photo_url text;
alter table leads add column if not exists client_photo_url_2 text;

-- Which of the two is the main point of contact — matters once there are
-- two names and Allison needs to know who to actually call/email first.
alter table leads add column if not exists primary_contact text
  not null default '1' check (primary_contact in ('1', '2'));
