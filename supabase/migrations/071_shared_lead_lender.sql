-- The client's loan/home-search page (/l/<token>) showed "Your agent" but
-- never the loan officer — get_shared_lead() simply didn't send one, even
-- when a lender_member_id was set on the lead (migration 039).
--
-- Adds a 'lender' object, sourced from the roster the same way 'realtor' is,
-- using the lender-side fields (company, NMLS, lender websites — never the
-- realtor websites, matching migration 036/042). Only public-facing contact
-- details, nothing internal. Everything else in the function is unchanged.
...
begin
  ...
  select jsonb_build_object(
    ...
    'lender', (
      select jsonb_build_object('full_name', m.full_name, 'headshot_url', m.headshot_url,
                                'phone', m.phone, 'email', m.email,
                                'company_name', m.company_name, 'nmls_number', m.nmls_number,
                                'is_mortgage_broker', ('mortgage_broker' = any(m.roles)
                                                       and not 'loan_officer' = any(m.roles)),
                                'website_1', m.lender_website_1, 'website_2', m.lender_website_2,
                                'website_3', m.lender_website_3)
        from team_members m where m.id = v_lead.lender_member_id
    ),
    'brand', (
