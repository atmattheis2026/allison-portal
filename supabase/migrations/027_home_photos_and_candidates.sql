-- Photos on both home lists, a private companion note alongside each
-- existing (client-visible) note, and a new "Homes you may like" list that,
-- like "Homes shown", is visible to the client and gets carried into
-- get_shared_lead().

alter table lead_homes add column if not exists photo_url text;
alter table lead_homes add column if not exists private_note text;
comment on column lead_homes.note is 'Client-visible.';
comment on column lead_homes.private_note is 'Internal only — never part of get_shared_lead().';

create table lead_maybe_homes (
  id             uuid primary key default gen_random_uuid(),
  lead_id        uuid not null references leads(id) on delete cascade,
  address_line   text not null default '',
  url            text,
  photo_url      text,
  note           text,          -- client-visible
  private_note   text,          -- internal only
  sort_order     int not null default 0
);
create index on lead_maybe_homes (lead_id, sort_order);

alter table lead_maybe_homes enable row level security;

create policy lead_child_rw on lead_maybe_homes for all to authenticated
  using (lead_visible(lead_maybe_homes.lead_id))
  with check (lead_visible(lead_maybe_homes.lead_id));
