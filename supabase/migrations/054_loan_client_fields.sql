-- Extends the existing Active Buyer lead into a general client record
-- instead of building a separate loan-client template that would need
-- merging later. wants_buying/wants_loan are independent — a client can
-- want either or both — and each reveals its own section on the same page.
-- Existing leads default to wants_buying = true, wants_loan = false, which
-- matches everything already in the system today.
alter table leads add column if not exists wants_buying boolean not null default true;
alter table leads add column if not exists wants_loan boolean not null default false;
alter table leads add column if not exists loan_type text
  check (loan_type in ('Conventional', 'FHA', 'VA', 'USDA', 'Jumbo', 'Other'));
alter table leads add column if not exists estimated_loan_amount numeric(12,2);
alter table leads add column if not exists loan_status text
  check (loan_status in ('Not started', 'Application submitted', 'In underwriting', 'Clear to close', 'Denied'));

-- Client sees loan type/status (useful progress info, same transparency
-- level as Qualification) but not the estimated amount, same reasoning as
-- keeping referral_transaction_fee_amount internal-only.
